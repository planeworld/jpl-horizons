...
objects["europa"]=502;
objects["io"]=501;
objects["ganymede"]=503;
objects["callisto"]=504;
objects["amalthea"]=505;
objects["himalia"]=506;
objects["elara"]=507;
objects["pasiphae"]=508;
objects["sinope"]=509;
...
function parseObjectData(data) {
    ...
    parser_mass.add(' Mass \\(10\\^([0-9]+) kg *\\) +[=~] +([0-9]*.[0-9]*)( \\(10\\^([+-][0-9]+)\\))?', function(match) {
        if (match[4])
            return parseFloat(match[2]+"E"+(parseInt(match[1])+parseInt(match[4])));
        else
            return parseFloat(match[2]+"E"+match[1]);
    });
